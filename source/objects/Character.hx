...
class Character extends OffsettableSprite
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		else if((flipMode && isPlayer) || (!flipMode && !isPlayer)) holdTimer = 0;

		if (((flipMode && isPlayer) || (!flipMode && !isPlayer)) && holdTimer >= Conductor.stepCrochet * (0.0011 #if FLX_PITCH / (FlxG.sound.music != null ? FlxG.sound.music.pitch : 1) #end) * singDuration)
		{
			dance();
			holdTimer = 0;
		}

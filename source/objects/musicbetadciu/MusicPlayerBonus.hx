...
*/
@:access(states.betadciu.BonusSongsState)
class MusicPlayerBonus extends FlxGroup 
{
 	public var instance:BonusSongsState;
 	...
 	public function new(instance:BonusSongsState)
 	{
 		super();
 	...
 	override function update(elapsed:Float)
 	{
 		...
 		var songName:String = instance.songs[BonusSongsState.curSelected].songName;
		...
		if (controls.UI_DOWN || controls.UI_UP)
		{
 		...
 		if (playing)
 		{
 			if(BonusSongsState.vocals != null)
 				BonusSongsState.vocals.volume = (BonusSongsState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 			if(BonusSongsState.opponentVocals != null)
 				BonusSongsState.opponentVocals.volume = (BonusSongsState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 
 			if((BonusSongsState.vocals != null && BonusSongsState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - BonusSongsState.vocals.time) >= 25) ||
 			(BonusSongsState.opponentVocals != null && BonusSongsState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - BonusSongsState.opponentVocals.time) >= 25))
 			{
 				pauseOrResume();
 	...
 	{
 		if (BonusSongsState.vocals != null && BonusSongsState.vocals.length > time)
 			BonusSongsState.vocals.time = time;
 		if (BonusSongsState.opponentVocals != null && BonusSongsState.opponentVocals.length > time)
 			BonusSongsState.opponentVocals.time = time;
 	}
 	...
 	public function pauseOrResume(resume:Bool = false) 
 	{
 		if (resume)
 		{
 			...
 			if (BonusSongsState.vocals != null && BonusSongsState.vocals.length > FlxG.sound.music.time && !BonusSongsState.vocals.playing)
 				BonusSongsState.vocals.resume();
 			if (BonusSongsState.opponentVocals != null && BonusSongsState.opponentVocals.length > FlxG.sound.music.time && !BonusSongsState.opponentVocals.playing)
 				BonusSongsState.opponentVocals.resume();
 		}
 		else 
 		{
 			...
 			if (BonusSongsState.vocals != null)
 				BonusSongsState.vocals.pause();
 			if (BonusSongsState.opponentVocals != null)
 				BonusSongsState.opponentVocals.pause();
 		}
 	}
 	...
 	{
 		var length:Int = instance.songs[BonusSongsState.curSelected].songName.length;
 	...
 	function setPlaybackRate() 
 	{
 		...
 		if (BonusSongsState.vocals != null)
 			BonusSongsState.vocals.pitch = playbackRate;
 		if (BonusSongsState.opponentVocals != null)
 			BonusSongsState.opponentVocals.pitch = playbackRate;
 	}

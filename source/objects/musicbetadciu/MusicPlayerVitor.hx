...
import states.betadciu.VitorState;
 
/**
  * Music player used for Freeplay
*/
@:access(states.betadciu.VitorState)
class MusicPlayerVitor extends FlxGroup 
{
 	public var instance:VitorState;
 	...
 	public function new(instance:VitorState)
 	{
 		super();
 	...
 	override function update(elapsed:Float)
 	{
 		...
 		var songName:String = instance.songs[VitorState.curSelected].songName;
		...
		if (controls.UI_DOWN || controls.UI_UP)
		{
 		...
 		if (playing)
 		{
 			if(VitorState.vocals != null)
				VitorState.vocals.volume = (VitorState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 			if(VitorState.opponentVocals != null)
				VitorState.opponentVocals.volume = (VitorState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 
 			if((VitorState.vocals != null && VitorState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - VitorState.vocals.time) >= 25) ||
 			(VitorState.opponentVocals != null && VitorState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - VitorState.opponentVocals.time) >= 25))
 			{
 				pauseOrResume();
 	...
 	{
 		if (VitorState.vocals != null && VitorState.vocals.length > time)
			VitorState.vocals.time = time;
 		if (VitorState.opponentVocals != null && VitorState.opponentVocals.length > time)
			VitorState.opponentVocals.time = time;
 	}
 	...
 	public function pauseOrResume(resume:Bool = false) 
 	{
 		if (resume)
 		{
 			...
 			if (VitorState.vocals != null && VitorState.vocals.length > FlxG.sound.music.time && !VitorState.vocals.playing)
				VitorState.vocals.resume();
 			if (VitorState.opponentVocals != null && VitorState.opponentVocals.length > FlxG.sound.music.time && !VitorState.opponentVocals.playing)
				VitorState.opponentVocals.resume();
 		}
 		else 
 		{
 			...
 			if (VitorState.vocals != null)
				VitorState.vocals.pause();
 			if (VitorState.opponentVocals != null)
				VitorState.opponentVocals.pause();
 		}
 	}
 	...
 	{
 		var length:Int = instance.songs[VitorState.curSelected].songName.length;
 	...
 	function setPlaybackRate() 
 	{
 		...
 		if (VitorState.vocals != null)
			VitorState.vocals.pitch = playbackRate;
 		if (VitorState.opponentVocals != null)
 			VitorState.opponentVocals.pitch = playbackRate;
 	}

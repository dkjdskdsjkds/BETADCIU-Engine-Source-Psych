...
import states.betadciu.NeonightState;
 
/**
  * Music player used for Freeplay
*/
@:access(states.betadciu.NeonightState)
class MusicPlayerNeo extends FlxGroup 
{
 	public var instance:NeonightState;
 	...
 	public function new(instance:NeonightState)
 	{
 		super();
 	...
 	override function update(elapsed:Float)
 	{
 		...
 		var songName:String = instance.songs[NeonightState.curSelected].songName;
		...
		if (controls.UI_DOWN || controls.UI_UP)
		{
 		...
 		if (playing)
 		{
 			if(NeonightState.vocals != null)
				NeonightState.vocals.volume = (NeonightState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 			if(NeonightState.opponentVocals != null)
				NeonightState.opponentVocals.volume = (NeonightState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 
 			if((NeonightState.vocals != null && NeonightState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - NeonightState.vocals.time) >= 25) ||
 			(NeonightState.opponentVocals != null && NeonightState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - NeonightState.opponentVocals.time) >= 25))
 			{
 				pauseOrResume();
 	...
 	{
 		if (NeonightState.vocals != null && NeonightState.vocals.length > time)
			NeonightState.vocals.time = time;
 		if (NeonightState.opponentVocals != null && NeonightState.opponentVocals.length > time)
			NeonightState.opponentVocals.time = time;
 	}
 	...
 	public function pauseOrResume(resume:Bool = false) 
 	{
 		if (resume)
 		{
 			...
 			if (NeonightState.vocals != null && NeonightState.vocals.length > FlxG.sound.music.time && !NeonightState.vocals.playing)
				NeonightState.vocals.resume();
 			if (NeonightState.opponentVocals != null && NeonightState.opponentVocals.length > FlxG.sound.music.time && !NeonightState.opponentVocals.playing)
				NeonightState.opponentVocals.resume();
 		}
 		else 
 		{
 			...
 			if (NeonightState.vocals != null)
				NeonightState.vocals.pause();
 			if (NeonightState.opponentVocals != null)
				NeonightState.opponentVocals.pause();
 		}
 	}
 	...
 	{
 		var length:Int = instance.songs[NeonightState.curSelected].songName.length;
 	...
 	function setPlaybackRate() 
 	{
 		...
 		if (NeonightState.vocals != null)
			NeonightState.vocals.pitch = playbackRate;
 		if (NeonightState.opponentVocals != null)
 			NeonightState.opponentVocals.pitch = playbackRate;
 	}

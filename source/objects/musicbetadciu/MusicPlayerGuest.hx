package objects.musicbetadciu;
 
import flixel.group.FlxGroup;
import flixel.ui.FlxBar;
import flixel.util.FlxStringUtil;
 
import states.betadciu.GuestBETADCIUState;
 
/**
  * Music player used for Freeplay
*/
@:access(states.betadciu.GuestBETADCIUState)
class MusicPlayerGuest extends FlxGroup 
{
 	public var instance:GuestBETADCIUState;
 	...
 	public function new(instance:GuestBETADCIUState)
 	{
 		super();
 	...
 	override function update(elapsed:Float)
 	{
 		...
 		var songName:String = instance.songs[GuestBETADCIUState.curSelected].songName;
		...
		{
			holdPitchTime = 0;
			if (FlxG.keys.pressed.SHIFT) playbackRate += 0.01; else playbackRate += 0.05;
			setPlaybackRate();
		}
		...
		if (controls.UI_DOWN || controls.UI_UP)
		{
			...
			{
				if (FlxG.keys.pressed.SHIFT) playbackRate += 0.01 * (controls.UI_UP ? 1 : -1); else playbackRate += 0.05 * (controls.UI_UP ? 1 : -1);
				setPlaybackRate();
			}
		}
 		...
 		if (playing)
 		{
 			if(GuestBETADCIUState.vocals != null)
				GuestBETADCIUState.vocals.volume = (GuestBETADCIUState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 			if(GuestBETADCIUState.opponentVocals != null)
				GuestBETADCIUState.opponentVocals.volume = (GuestBETADCIUState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;
 
 			if((GuestBETADCIUState.vocals != null && GuestBETADCIUState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - GuestBETADCIUState.vocals.time) >= 25) ||
 			(GuestBETADCIUState.opponentVocals != null && GuestBETADCIUState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - GuestBETADCIUState.opponentVocals.time) >= 25))
 			{
 				pauseOrResume();
 	...
 	{
 		if (GuestBETADCIUState.vocals != null && GuestBETADCIUState.vocals.length > time)
			GuestBETADCIUState.vocals.time = time;
 		if (GuestBETADCIUState.opponentVocals != null && GuestBETADCIUState.opponentVocals.length > time)
			GuestBETADCIUState.opponentVocals.time = time;
 	}
 	...
 	public function pauseOrResume(resume:Bool = false) 
 	{
 		if (resume)
 		{
 			...
 			if (GuestBETADCIUState.vocals != null && GuestBETADCIUState.vocals.length > FlxG.sound.music.time && !GuestBETADCIUState.vocals.playing)
				GuestBETADCIUState.vocals.resume();
 			if (GuestBETADCIUState.opponentVocals != null && GuestBETADCIUState.opponentVocals.length > FlxG.sound.music.time && !GuestBETADCIUState.opponentVocals.playing)
				GuestBETADCIUState.opponentVocals.resume();
 		}
 		else 
 		{
 			...
 			if (GuestBETADCIUState.vocals != null)
				GuestBETADCIUState.vocals.pause();
 			if (GuestBETADCIUState.opponentVocals != null)
				GuestBETADCIUState.opponentVocals.pause();
 		}
 	}
 	...
 	{
 		var length:Int = instance.songs[GuestBETADCIUState.curSelected].songName.length;
 	...
 	function setPlaybackRate() 
 	{
 		...
 		if (GuestBETADCIUState.vocals != null)
			GuestBETADCIUState.vocals.pitch = playbackRate;
 		if (GuestBETADCIUState.opponentVocals != null)
 			GuestBETADCIUState.opponentVocals.pitch = playbackRate;
 	}

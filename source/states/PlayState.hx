...
class PlayState extends MusicBeatState
{
	...
	public function addShaderToCamera(cam:String,effect:ShaderEffect) //STOLE FROM ANDROMEDA
	{
		switch(cam.toLowerCase()) 
		{
			...
			default:
				var obj = null;
				for (map in [modchartSprites, modchartCharacters]) {
					if (map.exists(cam)) {
						obj = map.get(cam);
						break;
					}
	...
	override public function update(elapsed:Float)
	{
		if(!inCutscene && !paused && !freezeCamera) {
			...
			var idleAnim:Bool = ((opponentPlay ? dad : boyfriend).getAnimationName().startsWith('idle') || (opponentPlay ? dad : boyfriend).getAnimationName().startsWith('danceLeft') || (opponentPlay ? dad : boyfriend).getAnimationName().startsWith('danceRight'));
	...
	public function endSong()
	{
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			else
			{
				...
				canResync = false;
				if (isBETADCIU) {
					if (Difficulty.list[0] == "Guest")
						MusicBeatState.switchState(new states.betadciu.GuestBETADCIUState());
					else
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		callOnScripts('onPopUpScore', [note]);
	}
	...
	public function cpuNoteHit(note:Note):Void {
		var result:Dynamic = callOnLuas('cpuNoteHitPre', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.dType]);
		...
		callOnLuas('dadPreNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.dType]);
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		callOnLuas('dadNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.dType]);
		callOnLuas('playerTwoSing', [Math.abs(note.noteData), Conductor.songPosition]);
		result = callOnLuas('opponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.dType]);
		// opponent hit
		stagesFunc(function(stage:BaseStage) opponentPlay ? stage.playerNoteHit(note) : stage.cpuNoteHit(note));
		var result:Dynamic = callOnLuas(opponentPlay ? 'playerNoteHit' : 'cpuNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.dType]);
	...
	public function playerNoteHit(note:Note):Void {
		...
		var result:Dynamic = callOnLuas('playerNoteHitPre', [notes.members.indexOf(note), leData, leType, isSus, note.dType]);
		...
		{
			callOnLuas('bfPreNoteHit', [notes.members.indexOf(note), leData, leType, isSus, note.dType]);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		callOnLuas('playerOneSing', [leData, Conductor.songPosition]);
		callOnLuas('bfNoteHit', [notes.members.indexOf(note), leData, leType, isSus, note.dType]);
		result = callOnLuas('goodNoteHit', [notes.members.indexOf(note), leData, leType, isSus, note.dType]);

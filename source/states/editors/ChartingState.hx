...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function update(elapsed:Float)
	{
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			for (note in curRenderedNotes)
			{
				...
				{
					var editorBFNote = note.mustPress;
			
					if(!note.extraData.get("playedEditorClick") && FlxG.sound.music.playing){
						if(editorBFNote){
	...
	function addChartingTab()
	{
		...
		objY += 25;
		lilBuddiesBox = new PsychUICheckBox(objX, objY, "Lil' Buddies", 100);
	...
	function addEventsTab()
	{
		...
		value3InputText = new PsychUIInputText(objX+ 150, objY + 35, 120, '', 8);

...
import objects.musicbetadciu.MusicPlayerNeo;
...
class NeonightState extends MusicBeatState
{
	...
	public var playerNeo:MusicPlayerNeo;
	...
	override function create()
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image('menuNEO'));
		...
		missingTextBG = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
 		...
 		playerNeo = new MusicPlayerNeo(this);
 		add(playerNeo);
	...
	{
		PlayState.isNeonight = true; //gotta move this cuz of the format thing
		...
		if (controls.BACK)
 		{
 			if (playerNeo.playingMusic)
 			{
 				...
 				playerNeo.playingMusic = false;
 				playerNeo.switchPlayMusic();
 		...
 		if(FlxG.keys.justPressed.CONTROL && !playerNeo.playingMusic)
 		{
 			persistentUpdate = false;
 		...
 		{
 			if(instPlaying != curSelected && !playerNeo.playingMusic)
 			{
 				destroyFreeplayVocals();
 				FlxG.sound.music.volume = 0;
 				...
 				playerNeo.playingMusic = true;
 				playerNeo.curTime = 0;
 				playerNeo.switchPlayMusic();
 				playerNeo.pauseOrResume(true);
 			}
 			else if (instPlaying == curSelected && playerNeo.playingMusic)
 			{
				playerNeo.pauseOrResume(!playerNeo.playing);
 			}
 		}
 		else if (controls.ACCEPT && !playerNeo.playingMusic)
 		{
 			persistentUpdate = false;
 		...
 		else if(controls.RESET && !playerNeo.playingMusic)
 		{
 			persistentUpdate = false;

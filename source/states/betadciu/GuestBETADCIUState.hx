...
import objects.musicbetadciu.MusicPlayerGuest;
...
{
	public var songs:Array<SongMetadata2> = [];
	...
	public var playerGuest:MusicPlayerGuest;
	...
	override function create()
	{
		...
		if (FlxG.sound.music.volume == 0 || !FlxG.sound.music.playing)
		{
			...
			FlxG.sound.playMusic(Paths.music('guest'));
		}
		...
		ytIcon.frames = Paths.getSparrowAtlas('betadciu/extraIcons');
		readXML(File.getContent(Paths.xmlNew('images/betadciu/extraIcons')));
		...
		missingTextBG = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
 		...
 		bg.color = songs[curSelected].textColor;
 		...
 		playerGuest = new MusicPlayerGuest(this);
 		add(playerGuest);
		...
		if (controls.BACK)
 		{
 			if (playerGuest.playingMusic)
 			{
 				...
 				playerGuest.playingMusic = false;
 				playerGuest.switchPlayMusic();
 		...
 		if(FlxG.keys.justPressed.CONTROL && !playerGuest.playingMusic)
 		{
 			persistentUpdate = false;
 		...
 		{
 			if(instPlaying != curSelected && !playerGuest.playingMusic)
 			{
 				destroyFreeplayVocals();
 				FlxG.sound.music.volume = 0;
 				...
 				playerGuest.playingMusic = true;
 				playerGuest.curTime = 0;
 				playerGuest.switchPlayMusic();
 				playerGuest.pauseOrResume(true);
 			}
 			else if (instPlaying == curSelected && playerGuest.playingMusic)
 			{
				playerGuest.pauseOrResume(!playerGuest.playing);
 			}
 		}
 		else if (controls.ACCEPT && !playerGuest.playingMusic)
 		{
 			persistentUpdate = false;
 		...
 		else if(controls.RESET && !playerGuest.playingMusic)
 		{
 			persistentUpdate = false;
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
 	{
 		...
 		var newColor:Int = songs[curSelected].textColor;

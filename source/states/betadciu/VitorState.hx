...
import objects.musicbetadciu.MusicPlayerVitor;
...
{
	public var songs:Array<SongMetadata3> = [];
	...
	public var playerVitor:MusicPlayerVitor;
	...
	override function create()
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image('menuBGTemplate'));
		...
		playerVitor = new MusicPlayerVitor(this);
		add(playerVitor);
	...
	{
		PlayState.isVitor = true; //gotta move this cuz of the format thing
		...
		if (controls.BACK)
		{
			if (playerVitor.playingMusic)
			{
				...
				playerVitor.playingMusic = false;
				playerVitor.switchPlayMusic();
		...
		if(FlxG.keys.justPressed.CONTROL && !playerVitor.playingMusic)
		{
			persistentUpdate = false;
		...
		{
			if(instPlaying != curSelected && !playerVitor.playingMusic)
			{
				destroyFreeplayVocals();
				FlxG.sound.music.volume = 0;
				...
				playerVitor.playingMusic = true;
				playerVitor.curTime = 0;
				playerVitor.switchPlayMusic();
				playerVitor.pauseOrResume(true);
			}
			else if (instPlaying == curSelected && playerVitor.playingMusic)
			{
				playerVitor.pauseOrResume(!playerVitor.playing);
			}
		}
		else if (controls.ACCEPT && !playerVitor.playingMusic)
		{
			persistentUpdate = false;
		...
		else if(controls.RESET && !playerVitor.playingMusic)
		{
			persistentUpdate = false;
